...
def get_followers():
    ...
    for follower in followers:
        ...
        found = (follower._json['id'], follower._json['screen_name'], follower._json['name'],
                 follower._json['profile_image_url_https'], follower._json['protected'], follower._json['verified'])
...
def get_following():
    ...
    for following in followings:
        ...
        found = (following._json['id'], following._json['screen_name'], following._json['name'],
                 following._json['profile_image_url_https'], following._json['protected'], following._json['verified'])
...
def follow_target(target_id):
    # Follow and get following user details
    user = api.create_friendship(id=target_id)
    username = user._json["screen_name"]
    id = user._json["id"]
    return user


def get_retweeters(user_id):
    global api
    retweeters = tweepy.Cursor(api.retweeters(id=user_id)).items()
    return retweeters


def get_user_tl(user_id):
    global api
    tl = tweepy.Cursor(api.user_timeline(user_id)).items()
    return tl
